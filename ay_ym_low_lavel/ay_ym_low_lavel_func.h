...
const ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >* ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::instance ( void ) {
    static const ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >obj;
    return &obj;
}


template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::init ( void ) const {
    ...
    USER_OS_STATIC_TASK_CREATE( this->task, "ay_low", 300, ( void* )this, TASK_PRIO, this->task_stack, &this->task_struct );
}
 ...
 */
template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::out_reg ( void ) const {
    this->cfg->sr->write();
    this->cfg->bc1->set();
    this->cfg->bdir->set();
    this->cfg->bdir->reset();
 ...
 */
template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::out_data ( void ) const {
    this->cfg->sr->write();
    this->cfg->bdir->set();
    this->cfg->bdir->reset();
}
 ...
 */
template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::timer_interrupt_handler ( void ) const {
 ...
 */
template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::queue_add_element ( ay_queue_struct* item ) const {
    ay_low_out_data buf;
    buf.reg     = item->reg;
    buf.data    = item->data;
...
template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::set_channel ( uint8_t number_ay, uint8_t channel, bool set ) const {
...
template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::hardware_clear ( void ) const {
    /*
 ...
 */
template < uint8_t AY_NUMBER, uint8_t TASK_PRIO, const ay_ym_low_lavel_cfg_t* const STRUCT_INIT >
void ay_ym_low_lavel< AY_NUMBER, TASK_PRIO, STRUCT_INIT >::task ( void* p_this ) {
        ...
        while( true ) {
            ...
            xSemaphoreTake ( obj->semaphore, portMAX_DELAY );   // Как только произошло прерывание (была разблокировка из ay_timer_handler).
                while (flag != (0xFFFFFFFF >>(32-AY_NUMBER)) ) {    // Выдаем данные, пока все очереди не освободятся.]
                     ...
                     */
                    for ( volatile uint8_t chip_loop = 0; chip_loop < AY_NUMBER; chip_loop++ ) {    // Собираем регистр/данные со всех очередей всех чипов.
                        ...
                        if ( count != 0) {    // Если для этого чипа очередь не пуста.
                                ...
                                if ( buffer[chip_loop].reg == 0xFF ){    // Если это флаг того, что далее читать можно лишь в следущем прерывании,...
                                    ...
                                    if (  buffer[chip_loop].reg == 7){            // Сохраняем состояние 7-го регситра разрешения генерации звука и шумов. Чтобы в случае паузы было что вернуть. После затирания 0b111111 (отключить генерацию всего).
                                        obj->r7_reg[chip_loop] =  buffer[chip_loop].reg;
                                    }
                                }
                            } else {
             ...
             */
         /*   obj->cfg->tic_ff++;
            if (obj->cfg->tic_ff == 50){        // Если насчитали секунду.
                obj->cfg->tic_ff = 0;
                if (obj->cfg->semaphore_sec_out != NULL){    // Если есть соединение семофором, то отдать его.
...
void ay_play_stait (int fd, uint8_t stait){
    ...
    if (stait){
        ...
        for (int loop_ay = 0; loop_ay<AY_NUMBER, TASK_PRIO, STRUCT_INIT; loop_ay++){        // Возвращаем состояние всех AY.
...
void ay_music_off (int fd){
    ...
    uint16_t buf;
    for (int loop_ay = 0; loop_ay<AY_NUMBER, TASK_PRIO, STRUCT_INIT; loop_ay++){        // Извлекаем данные из всех очередей.
